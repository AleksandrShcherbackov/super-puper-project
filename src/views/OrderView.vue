...
<template>
  <div class="container mx-auto flex items-center justify-center p-12">
    <div class="m-auto flex flex-col border border-cyan-800 p-4 rounded max-w-3xl">
      <h2 class="text-lg font-semibold text-gray-800 mx-auto">
        {{ $t('order.form.tarrifChoose') }}
      </h2>
      <div class="tariff-list">
        <TarifSelector :tariffs="tariffs" @changeTariff="selectTariff" />
      </div>

      <h2 class="text-lg font-semibold text-gray-800 mt-4 mx-auto">
        {{ $t('order.form.submit') }}
      </h2>
      <form @submit.prevent="submitForm" class="max-w-lg mx-auto p-6 bg-white shadow-md rounded-lg">
        <div class="form-group mb-4">
          ...
          />
          <span v-if="errors.fullName" class="text-red-500 text-sm mt-1">{{
            errors.fullName
          }}</span>
        </div>
        ...
        <div class="form-group mb-4">
          <label for="cardNumber" class="block text-gray-700 font-semibold mb-2"
            >{{ $t('order.form.cardNumber') }}:</label
          >
          <input
            type="text"
            id="cardNumber"
            ...
            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
          <span v-if="errors.cardNumber" class="text-red-500 text-sm mt-1">{{
            errors.cardNumber
          }}</span>
        </div>
        ...
        <div class="flex space-x-4 mb-4">
          <div class="flex-1">
            <label for="expiryDate" class="block text-gray-700 font-semibold mb-2"
              >{{ $t('order.form.date') }}:</label
            >
            <input
              type="text"
              id="expiryDate"
              ...
              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
            <span v-if="errors.expiryDate" class="text-red-500 text-sm mt-1">{{
              errors.expiryDate
            }}</span>
          </div>
